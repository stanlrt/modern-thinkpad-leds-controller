...
// LedBlinkMonitor provides software-based blinking for LEDs set to Blink mode.
// Some ThinkPad LEDs (like FnLock) don't support hardware blinking (0xC0 command),
// so we need to manually toggle them between On and Off at a regular interval.
//
// This monitor tracks which LEDs are in Blink mode and actively toggles them.
public sealed class LedBlinkMonitor : IDisposable
{
    ...
    public LedBlinkMonitor(LedController leds, int blinkIntervalMs = 500)
    {
        _leds = leds;
